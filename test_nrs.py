...
def test_pipeline_esa(slit_wcs, trace_file):
    ...
    det2msa = slit_wcs.get_transform('detector', 'slit_frame')
    ...
    try:
        ...
        print('Slice {0}, max diff in wavelength: '.format(slice_id), np.abs(diff[close]).max())
    ...
    try:
        ...
        #print('Number of pixels with NaN values in the diff: ', len(not_close[0]))
        #print('Number of pixels with values of <= 10**-13 in the diff: ', len(l[close]))
        print('Slice {0}, max diff along slit y-axis: '.format(slice_id), np.abs(slit_diff[close]).max())
    ...
    try:
        ...
        #print('Number of pixels with NaN values in the diff: ', len(not_close[0]))
        #print('Number of pixels with values of <= 10**-13 in the diff: ', len(l[close]))
        print('Slice {0}, max diff in MSA x coordinate: '.format(slice_id), np.abs(msay_diff[close]).max())
    ...
    try:
        ...
        #print('Number of pixels with NaN values in the diff: ', len(not_close[0]))
        #print('Number of pixels with values of <= 10**-13 in the diff: ', len(l[close]))
        print('Slice {0}, max diff in MSA y coordinate: '.format(slice_id), np.abs(msay_diff[close]).max())
